...
<div class="container w-50" style="margin-top: 80px">
  <form action="{{route('comments.store')}}" method="post" class="d-flex">
    @csrf
    <input type="text" name="comment" required class="form-control" style="margin-right: 5px;" placeholder="Add your comment" />
    ...
    <div class="col-md-12 mt-2">
      <div class="card">
        <div class="card-header d-flex align-items-center justify-content-between">
          ...
          <form action="{{route("comments.destroy", $comment->id)}}" method="post" class="btn-group" role="group">
            @csrf
            @method('DELETE')
            <button type="button" class="btn btn-warning" data-bs-toggle="collapse" data-bs-target="#{{$comment->user->email . $comment->id}}" aria-expanded="false" aria-controls="{{$comment->user->email . $comment->id}}">
