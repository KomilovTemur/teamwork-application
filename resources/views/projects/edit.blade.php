  ...
  <div class="row">
    <div class="col-md-8">
      <form action="{{ route('projects.update', $project->id) }}" method="post" class="d-flex w-100 flex-column">
        ...
        <label>
          Name
          <input class="form-control mb-2" type="text" value="{{ $project->name }}" name="name" id="">
        </label>
        <label>
          Description
          <textarea class="form-control mb-2" type="text" value="" name="description" id="" cols="30" rows="5">{{ $project->description }}</textarea>
        </label>
        <div class="d-flex">
      ...
      <form action="/project/addMember" method="post" class="mt-3">
        ...
        <input required="" type="text" name="username" placeholder="Add Member (username)" class="form-control mb-2">
    ...
    <div class="col-md-4">
      ...
      <ul class="list-group mt-2">
        ...
        <li class="list-group-item d-flex align-items-center justify-content-between">
          ...
          @endif
          {{$project->user[auth()->id() - 1]->id == auth()->id() && $project->user[$loop->index]->id == auth()->id()}}

            @if ($project->user[auth()->id() - 1]->id == auth()->id() && $project->user[$loop->index]->id == auth()->id())
            <form action="#">
